...
#define MMGR_GET_PART_MODE(part) ((((mMemoryPart_t *)(part))->partInfo >> 1) & 1)
#define MMGR_SET_PART_OCCUPY(part, occupy) (((mMemoryPart_t *)(part))->partInfo = ((mMemoryPart_t *)(part))->partInfo & ~1 | (occupy))
#define MMGR_SET_PART_MODE(part, mode) (((mMemoryPart_t *)(part))->partInfo = ((mMemoryPart_t *)(part))->partInfo & ~2 | ((mode) << 1))
...
void mMemoryMgrInit(size_t memoryPoolSize, size_t spiltParts, size_t partsMode)
{
    ...
    for(int i = 0; i < spiltParts; i++){
        ...
        MMGR_SET_PART_MODE(newPart, (partsMode >> i) & 1);
    ...
    for(curPart = mMemoryMgr->parts; curPart != NULL; curPart = curPart->next){
        ...
        if(namespace){
            ...
            if(!MMGR_GET_PART_MODE(curPart)){
                ...
                mMemoryNs_t *newNs = (mMemoryNs_t *)curPart->addr;
            ...
            rb_erase(&namespace->node, &curPart->recycleRoot);
            mMemoryNs_t *newNs = (mMemoryNs_t *)(curPart->addr + curPart->leftSpace - sizeof(mMemoryNs_t));
            memset((void *)newNs, 0, sizeof(mMemoryNs_t));
            newNs->size = size;
